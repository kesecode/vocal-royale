...
describe('song-choice/api GET', () => {
	...
	it('403 for non-participants', async () => {
		...
		const res = await GET(
			makeEvent('/song-choice/api', { user }) as unknown as Parameters<typeof GET>[0]
		)
	...
	it('returns 5 slots and fills existing choices', async () => {
		...
		const res = await GET(
			makeEvent('/song-choice/api', { user, pb }) as unknown as Parameters<typeof GET>[0]
		)
		expect(res.status).toBe(200)
		const body = (await jsonOf(res)) as {
			songs: Array<{ artist?: string; songTitle?: string; confirmed?: boolean }>
		}
...
describe('song-choice/api POST', () => {
	it('401 for unauthenticated users', async () => {
		const res = await POST(
			makeEvent('/song-choice/api', { method: 'POST', body: {} }) as unknown as Parameters<
				typeof GET
			>[0]
		)
		expect(res.status).toBe(401)
	...
	it('403 for non-participants', async () => {
		const user = makeUser({ role: 'spectator' })
		const res = await POST(
			makeEvent('/song-choice/api', { method: 'POST', user, body: {} }) as unknown as Parameters<
				typeof GET
			>[0]
		)
		expect(res.status).toBe(403)
	...
	it('validates fields', async () => {
		...
		const res = await POST(
			makeEvent('/song-choice/api', {
				method: 'POST',
				user,
				body: { round: 1, artist: '', songTitle: '' }
	...
	it('verifies via Apple and creates choice when none exists', async () => {
		...
		const res = await POST(
			makeEvent('/song-choice/api', { method: 'POST', user, pb, body }) as unknown as Parameters<
				typeof GET
			>[0]
		)
		expect(res.status).toBe(200)
