...
export const POST: RequestHandler = async ({ locals, request }) => {
  ...
  try {
    if (action === 'start_competition') {
      ...
      return json({ ok: true, state: updated, activeParticipant: active ? { id: active.id, name: toName(active), artistName: active.artistName } : null });
    }
    ...
    if (action === 'activate_rating_phase') {
      ...
      return json({ ok: true, state: updated, activeParticipant: { id: active.id, name: toName(active), artistName: active.artistName, sangThisRound: true } });
    }
    ...
    if (action === 'next_participant') {
      ...
      const updated = await upsertState(locals, { roundState: 'singing_phase', activeParticipant: picked.id });
      logger.info('Admin API: next_participant', { activeParticipant: picked.id });
      return json({ ok: true, state: updated, activeParticipant: { id: picked.id, name: toName(picked), artistName: picked.artistName } });
    }
    ...
    if (action === 'show_results') {
      ...
      type Row = { id: string; name: string | null; artistName?: string; avg: number; sum: number; count: number; eliminated: boolean };
      const rows: Row[] = participants.map((p) => {
        const g = grouped.get(p.id) || { sum: 0, count: 0 };
        const avg = g.count > 0 ? g.sum / g.count : 0;
        return { id: p.id, name: toName(p), artistName: p.artistName, avg, sum: g.sum, count: g.count, eliminated: false };
      });
      ...
      if (toEliminate.length > 0) {
        await Promise.all(
          toEliminate.map((r) => locals.pb.collection(USERS_COLLECTION).update(r.id, { eliminated: true }))
        );
    ...
    if (action === 'reset_game') {
      ...
      if (current) {
        ...
        updated = (await locals.pb.collection(STATE_COLLECTION).create(createData)) as CompetitionStateResponse;
      }
