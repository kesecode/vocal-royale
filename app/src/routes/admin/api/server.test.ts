...
describe('admin/api access', () => {
  it('GET 403 when not admin', async () => {
    const res = await GET(makeEvent('/admin/api', { user: makeUser({ role: 'spectator' }) }) as unknown as Parameters<typeof GET>[0])
    expect(res.status).toBe(403)
  })

  it('POST 403 when not admin', async () => {
    const res = await POST(makeEvent('/admin/api', { method: 'POST', user: makeUser({ role: 'juror' }), body: { action: 'reset_game' } }) as unknown as Parameters<typeof POST>[0])
    expect(res.status).toBe(403)
  })
})
...
describe('admin/api actions', () => {
  it('activate_rating_phase -> 400 when no active participant', async () => {
    ...
    const res = await POST(makeEvent('/admin/api', { method: 'POST', user: admin, pb, body: { action: 'activate_rating_phase' } }) as unknown as Parameters<typeof POST>[0])
  ...
  it('start_competition picks a participant and starts singing phase', async () => {
    const admin = makeUser({ role: 'admin' })
    const participants = [
      makeUser({ id: 'p1', role: 'participant', eliminated: false, sangThisRound: false, name: 'A' }),
      makeUser({ id: 'p2', role: 'participant', eliminated: false, sangThisRound: false, name: 'B' })
    ]
    const pb = createPBMock({
      users: {
    ...
    const res = await POST(makeEvent('/admin/api', { method: 'POST', user: admin, pb, body: { action: 'start_competition' } }) as unknown as Parameters<typeof POST>[0])
    expect(res.status).toBe(200)
    const data = await jsonOf(res) as { ok: boolean; state: { roundState: string; activeParticipant: string | null | undefined } }
