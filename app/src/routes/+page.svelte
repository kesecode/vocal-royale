        ...
        <div class="p-3 sm:p-4">
            {#if data?.winner}
                ...
                {#if data.winner.avg !== undefined}
                    <div class="text-sm text-white/80">Ø Bewertung: {data.winner.avg.toFixed(2)}{#if data.winner.count} (Stimmen: {data.winner.count}){/if}</div>
                {/if}
            {:else}
            ...
            <table class="w-full text-sm">
                ...
                <tbody>
                    {#each data.participants as u}
                        <tr class="align-middle border-t border-[#333]/40">
