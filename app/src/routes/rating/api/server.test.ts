...
function makeEvent(pathname: string, opts?: Partial<MakeEventOptions>) {
  ...
  const reqInit: RequestInit = opts?.method === 'POST'
    ? { method: 'POST', headers: { 'content-type': 'application/json' }, body: opts?.body ? JSON.stringify(opts.body) : undefined }
    : { method: 'GET' }
...
describe('rating/api GET', () => {
  ...
  it('returns participants and existing ratings for juror', async () => {
    ...
    const body = await jsonOf(res) as { round: number; participants: { id: string }[]; ratings: unknown[] }
...
describe('rating/api POST', () => {
  ...
  it('400 for invalid JSON body', async () => {
    ...
    const request = new Request(url, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{invalid' })
    const res = await POST({ request, url, locals: { user, pb: createPBMock() } } as unknown as Parameters<typeof POST>[0])
    expect(res.status).toBe(400)
  })

  it('validates inputs and rejects self rating', async () => {
    const user = makeUser({ id: 'me', role: 'spectator' })
    const pb = createPBMock({ competition_state: { getList: async () => ({ items: [{ roundState: 'rating_phase' }] }) } })
  ...
  it('rejects when rating phase is closed', async () => {
    const user = makeUser({ role: 'spectator' })
    const pb = createPBMock({ competition_state: { getList: async () => ({ items: [{ roundState: 'singing_phase' }] }) } })
    const body = { round: 1, ratedUser: 'p1', rating: 3 }
    const res = await POST(makeEvent('/rating/api', { method: 'POST', user, pb, body }) as unknown as Parameters<typeof POST>[0])
    expect(res.status).toBe(400)
