...
const paint = (s: string, color: keyof typeof codes) => colorEnabled ? codes[color] + s + codes.reset : s;

function fmtLevel(level: LevelName): string {
  switch (level) {
    case 'error': return paint('✖ ERROR', 'red');
    case 'warn': return paint('⚠ WARN', 'yellow');
    case 'info': return paint('ℹ INFO', 'cyan');
    case 'debug': return paint('• DEBUG', 'magenta');
  }
}
...
function fmtMeta(meta?: Record<string, unknown>): string {
  ...
  const parts = keys.sort().map((k) => {
    const v = rest[k];
    const rendered = (typeof v === 'string' || typeof v === 'number' || typeof v === 'boolean' || v == null)
      ? String(v)
